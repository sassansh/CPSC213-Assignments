...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
  //reset list length to 0 to map to correct spot
  out_list->len = 0;

  //for each element in in_list, process it with *f and store value in out_list
  for (int i = 0; i < in_list->len; i++) {
    element_t newValue = 0; //Value to store result
    f (&newValue, in_list->data [i]); //Pass data[i] and pointer of newValue to f
    list_append (out_list, newValue);// Add new Value to out_list with list_append
  }
}
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
  ...
  //figure out min length of the two lists
  if (in_list0->len < in_list1->len) {
    minLength = in_list0->len;
  } else {
    minLength = in_list1->len;
  }

  //iterate over the elements of the lists, passing them to f, storing result in out_list
  for (int i = 0; i < minLength; i++) {
    element_t newValue = 0;
    f (&newValue, in_list0->data [i], in_list1->data [i]);
    list_append (out_list, newValue);
  }
}
...
void list_filter (int (*f) (element_t), struct list* out_list, struct list* in_list) {
  //reset list length to 0 to append to correct spot
  out_list->len = 0;

  //iterate over each item in in_list and append only if *f returns true for it
  for (int i = 0; i < in_list->len; i++)
    if (f (in_list->data [i])){
